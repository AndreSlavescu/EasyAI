...
void transpose_nxn_baseline(float* src, float* dst, int n) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            dst[j * n + i] = src[i * n + j];
        }
...
void transpose_nxn_avx2_128bit(float* src, float* dst, int n) {
...
void transpose_nxn_avx2_256bit_kernel1(float* src, float* dst, int n) {
    #pragma omp parallel for collapse(2)
    for (int i = 0; i < n; i += 8) {
        for (int j = 0; j < n; j += 8) {
...
*/
void transpose_nxn_avx2_256bit_kernel2(float* src, float* dst, int n) {
...
int main() {
    ...
    if (!is_correct_128bit || !is_correct_256bit_kernel1 || !is_correct_256bit_kernel2) return EXIT_FAILURE;
